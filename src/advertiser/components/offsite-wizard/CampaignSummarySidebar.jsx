...
export function CampaignSummarySidebar({ campaignData, completedSteps, onStepClick }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, fontFamily: "'Open Sans', sans-serif" }}>
      {/* Reach estimate */}
      {reach && (
        <StatCard
      ...
      {/* Summary card */}
      <div style={{
        background: 'var(--osmos-bg-subtle)', border: `1px solid var(--osmos-border)`, borderRadius: 10, padding: 12,
      }}>
        ...
        {completedEntries.length === 0 ? (
          <p style={{ fontSize: 12, color: 'var(--osmos-fg-subtle)', lineHeight: 1.5, margin: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {completedEntries.map(entry => (
              ...
              >
                <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--osmos-fg-subtle)', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  {entry.label}
                </p>
                <p style={{ fontSize: 12, color: 'var(--osmos-fg)', margin: 0, fontWeight: 500 }}>
                  {entry.summary}
                </p>
              </button>
            ))}
